...
// ─── SELL VIEW ────────────────────────────────────────────────────────────────
function SellView({item,onConfirm,onBack,loading,isLaBaraka=false}) {
  ...
  return (
    <div>
      ...
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:16,marginBottom:12}}>
        ...
        {/* La Baraka : Paiement CB/Espèces | Autres orgs : Canal Store/Online */}
        {isLaBaraka ? (
          <div style={{marginBottom:14}}>
...
function VendeurApp({ profile, onSignOut }) {
  ...
  const [isLaBaraka,setIsLaBaraka]   = useState(false); // Feature flag — CB/Espèces visible uniquement si partenaire = La Baraka
  ...
  const load = useCallback(async()=>{
    ...
    try {
      ...
      if (settings.partner_name) {
        setIsLaBaraka(settings.partner_name.trim().toLowerCase() === "la baraka");
      }
    }
      ...
      <div style={{background:C.bg,minHeight:"100vh",color:C.text,fontFamily:"'Space Grotesk',sans-serif",maxWidth:480,margin:"0 auto"}}>
        ...
        <div style={{padding:"16px 16px 96px"}}>
          ...
          {!loading&&!error&&<>
            ...
            {screen==="sell"&&selected&&<SellView item={selected} onConfirm={handleSell} onBack={goBack} loading={saving} isLaBaraka={isLaBaraka}/>}
