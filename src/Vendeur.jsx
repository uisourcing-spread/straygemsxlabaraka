...
import { supabase, signIn, signOut, getProfile, fetchAll, createRecord, updateRecord, updateRecordVendor, updateRecordVendorEdit, fetchSettings, writeLog } from "./supabase";
  ...
  return (
    <div>
      ...
      <button onClick={()=>onConfirm({...item,status:"sold",saleDate,channel,
        // Si pas de réduction, on passe sellPrice et non null — null ferait crasher Postgres
        finalPrice:hasRed&&priceNum!==item.sellPrice?priceNum:item.sellPrice,
        paymentMethod})}
...
function VendeurApp({ profile, onSignOut }) {
  ...
  const showChannel       = useMemo(() => !showPaymentMethod, [showPaymentMethod]);
  ...
  const load = useCallback(async()=>{
    ...
    try {
      ...
      // payment_method_enabled géré via org_id hardcodé — session 3 : rendre dynamique
    }
  ...
  // Guard — SellView force status:"sold" dans data avant l'appel,
  // donc on ne peut pas se fier à data.status pour détecter une pièce déjà vendue.
  // On checke le statut de l'item original dans le state local.
  const originalItem = items.find(i => i.id === data.id);
  if (!originalItem || originalItem.status === "sold") { 
  ...
  const handleEdit = async (data)=>{
    ...
    setSaving(true);
    try {
      const u=await updateRecordVendorEdit(data.id,data);
      setItems(p=>p.map(i=>i.id===u.id?u:i));
      writeLog("item.updated",{ref:data.ref,name:data.name,fields:["name","category","sellPrice","depositDate"]});
      showToast("Pièce mise à jour");
      goBack();
    } catch(e){ showToast("Oops — une erreur est survenue. Contacte un admin.","error"); }
