...
// Vendeur — champs éditables (nom, catégorie, prix catalogue, date dépôt)
const toDBVendorEdit = (item) => ({
  name:         item.name,
  category:     item.category    || null,
  deposit_date: item.depositDate || null,
  sell_price:   item.sellPrice   || null,
...
// Vendeur — champs vente uniquement (status, final_price, channel, payment_method)
const toDBVendorUpdate = (item) => ({
  status:        item.status,
  sale_date:     item.saleDate      || null,
  // Sécurisation explicite : || null échoue si finalPrice vaut 0 (falsy)
  // On accepte uniquement un nombre valide, sinon null
  final_price:   (item.finalPrice != null && !isNaN(item.finalPrice))
...
export const updateRecordVendorEdit = async (id, item) => {
  const { data, error } = await supabase
    .from("items")
    .update(toDBVendorEdit(item))
    .eq("id", id)
    .select()
    .single();
  if (error) throw new Error(error.message);
...
export const fetchSettings = async () => {
  // Récupère l'org_id du profil connecté pour filtrer les settings par organisation
  const { data: { session } } = await supabase.auth.getSession();
...
export const upsertSetting = async (key, value) => {
  // Récupère l'org_id pour écrire le setting dans la bonne organisation
  const { data: { session } } = await supabase.auth.getSession();
  if (!session) throw new Error("Non authentifié");
  const { data: profile } = await supabase
    .from("profiles")
  ...
  const { data, error } = await supabase
    .from("settings")
    .upsert({ key, value: String(value), org_id: orgId }, { onConflict: "key,org_id" })
    .select()
    .single();
  if (error) throw new Error(error.message);
  return data;
