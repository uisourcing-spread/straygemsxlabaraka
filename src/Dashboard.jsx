...
function LoginGate({ onUnlock }) {
  ...
  const tryLogin = async () => {
    ...
    try {
      ...
      writeLog("auth.login", { role: "admin", email: user.email });
      onUnlock(profile);
    } catch(e) {
      setError(e.message);
      setShake(true);
...
// ─── EDITABLE TABLE ROW ───────────────────────────────────────────────────────
const InventoryTable = ({items, onEdit, onDelete, highlightOldIds=[], highlightNewIds=[], catColors={}}) => {
    ...
    <div style={{background:C.bg,minHeight:"100vh",color:C.text,fontFamily:"'Space Grotesk',sans-serif"}}>
      <style dangerouslySetInnerHTML={{__html:`
        @media(max-width:768px){
          .db-header-inner{flex-wrap:wrap;height:auto;padding:12px 0;gap:8px;}
          .db-nav{display:none;}
      ...
      <div style={{maxWidth:1280,margin:"0 auto",padding:"28px 32px"}}>
        ...
        {!loading && view==="dashboard" && <>
          {/* KPIs */}
          <div className="db-kpis" style={{display:"flex",gap:12,flexWrap:"wrap"}}>
        ...
        {view==="team" && (()=>{
          ...
          const handleInvite = async () => {
            ...
            try {
              ...
              await inviteVendor(inviteEmail.trim(), profile.org_id);
